...
The functional requirements are straight forward and listed in \cref{tab:funcReq}.
    ...
    ID  & Name                                                                & Classification & Description                                                                                                                        \\ \hline
    ...
    103 & \begin{tabular}[c]{@{}l@{}}Low energy \\ transmission\end{tabular}  & Must           & The microcontroller and its  transmission partner must be able to communicate via a low energy transmission protocol               \\
    ...
    105 & \begin{tabular}[c]{@{}l@{}}Wake up \\ from sensor\end{tabular}      & Must           & The microcontroller must be able to wake up from an external sensor input.                                                         \\
    106 & \begin{tabular}[c]{@{}l@{}}Wake up\\ from timeout\end{tabular}      & Must           & The microcontroller must be able to wake up after a predefined timeout.                                                            \\
    107 & \begin{tabular}[c]{@{}l@{}}Transmission\\ Range\end{tabular}        & Must           & The transmission range must be larger than 5 meters without blockage.                                                              \\
    108 & Pin reads                                                           & Must           & The microcontroller must support reading from input pins.                                                                          \\
    109 & \begin{tabular}[c]{@{}l@{}}Centralized\\ configuration\end{tabular} & Should         & All configurations should be synchronized across the nodes in the system.                                                          \\
    110 & \begin{tabular}[c]{@{}l@{}}Node fault\\ tolerance\end{tabular}      & Should         & The state of each node should be synchronized in the system, so that in case of an error, the last operable state can be recovered \\
    111 & \begin{tabular}[c]{@{}l@{}}Adjustable\\ Goals setup\end{tabular}    & Should         & The goals setup need to able to adjust in their width to cover a variety of different goal sizes                                   \\
    112 & \begin{tabular}[c]{@{}l@{}}Containers\\ (excl microc.)\end{tabular} & Should         & All applications, excluding the microcontroller) should run inside containers.                                                     \\
    113 & Containers                                                          & Would          & The microcontroller code runs inside containers as well.                                                                           \\
    114 & \begin{tabular}[c]{@{}l@{}}Single \\ binaries\end{tabular}          & Could          & All applications should be packaged in one binary.                                                                                 \\
    115 & \begin{tabular}[c]{@{}l@{}}Edge\\ storage\end{tabular}              & Would          & The edge part should be able to count goals and syncronize with user devices without the Internet.                                 \\    
